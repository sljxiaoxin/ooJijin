   ...
   public:
      ...
      static void Init(int fast, int slow, int slower){
         ...
         for(int i=0; i<s_limit; i++){
            ma10Overlying_buffer[i]  = iMA(NULL,0,s_openMa,0,MODE_EMA,PRICE_CLOSE,i);
            ma120Overlying_buffer[i] = iMA(NULL,0,120,0,MODE_EMA,PRICE_CLOSE,i);

         }
         s_ma10              = iMA(NULL,0,s_openMa,0,MODE_EMA,PRICE_CLOSE,1);
         s_ma10_pre          = iMA(NULL,0,s_openMa,0,MODE_EMA,PRICE_CLOSE,2);
         s_ma10Overlying     = iMAOnArray(ma10Overlying_buffer,s_limit,s_openMa,0,MODE_EMA,1);
         s_ma10Overlying_pre = iMAOnArray(ma10Overlying_buffer,s_limit,s_openMa,0,MODE_EMA,2);
         ...
         for(int j=0; j<10; j++){
            s_ma10Overlying_buffer[j]  = iMAOnArray(ma10Overlying_buffer,s_limit,s_openMa,0,MODE_EMA,j);
         }
      ...
      static double GetMa10(int index){
         return iMA(NULL,0,s_openMa,0,MODE_EMA,PRICE_CLOSE,index);
      }
      ...
      static double GetMa10Overlying(int index){
         return s_ma10Overlying_buffer[index];//iMAOnArray(s_ma10Overlying_buffer,s_limit,10,0,MODE_EMA,index);
      }
